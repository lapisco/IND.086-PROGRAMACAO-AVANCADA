...
def storage():
    '''
    * storage: Store positive results added by the user. *

    Parameters:
        None: None
            The function prompts you to enter a floating value,
            the value will be stored in an array and stores the values in the array until you find the flag that is the stop condition,
            the entry of a negative value. (Type: float)
    Returns:
        The function returns an integer value with the added numbers and a vector with all the numbers, or returns 0 and an empty vector,
        if the first number is negative, which is the stop condition. (Type: Integer and List)
...
def main():
    ...
    _, grade = storage()
    s, _, vH2, _, vL2 = hls(3, grade, True)
    
    print('\nThe total number is {},\nthe biggest number is {},\nthe lowest note is {},\nand the sum of all numbers is {},\nand the average of the numbers is {}'.format(len(grade), vH2, vL2, s, (s/(len(grade)))))
